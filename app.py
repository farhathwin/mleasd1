...
def customer_leads_list():
    ...
    if request.method == 'POST':
        selected_lead_id = request.form.get('selected_lead')
...
def update_leads(lead_id):
    ...
    print(f"Querying for lead_id: {lead_id}, company_id: {company_id}")
    query = Lead.query \
        .join(Customer, (Lead.customer_id == Customer.customer_id) & (Customer.company_id == company_id)) \
        .filter(Lead.lead_id == lead_id, Lead.company_id == company_id) \
        .options(joinedload(Lead.customer))
    print(str(query))  # This will print the raw SQL query generated by SQLAlchemy
    lead = query.first()

    if lead:
        print(f"Found lead with ID {lead.lead_id} and customer {lead.customer.full_name}")
    else:
        print("No lead found.")
